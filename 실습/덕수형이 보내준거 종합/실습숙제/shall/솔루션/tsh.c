...
void eval(char *cmdline) 
{
		...
		if( !builtin_cmd( apszBuffer ) ) // apszBuffer의 값이 builtin_cmd에 있으면 싫행
		{
				...
				if(nFD != -1) // open에 실패하지 않았다면
						close(nFD); // 파일 디스크립터를 닫아준다.
				...
				else // nState가 BG라면 BG를 출력시킴
						printf("[%d] (%d) %s", pid2jid(pid), pid, jobs[maxjid(jobs) - 1].cmdline);
		}
}
...
int builtin_cmd( char** apszBuffer ) // built in 명령어들을 실행하게 해줌
{
		...
		if( !strcmp(apszBuffer[0], "quit") ) // apszBuffer에 quit이 들어있다면
				exit(0); // 쉘을 종료시킴
		...
		if(!strcmp(apszBuffer[0], "bg")) // apszBuffer에 "bg"가 있으면
		{
				if(apszBuffer[1][0] == '%') // jid가 입력되는 경우엔
				{
						...
						pid = atoi(apszBuffer[1]); // 바로 pid에 저장
